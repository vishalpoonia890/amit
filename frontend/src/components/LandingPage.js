...
import promoImage from '../assets/ipbi.png'; // ipbi.png will be shown on click

const AccordionItem = ({ title, children, isOpen, onClick }) => (
  <div className="faq-item">
    <button className="faq-header" onClick={onClick} aria-expanded={isOpen}>
      <span>{title}</span>
...
const PromoModal = ({ onClose, onRegisterClick }) => (
  <div className="promo-modal-overlay" onClick={onClose} role="dialog" aria-modal="true">
...
function LandingPage({
  authView, setAuthView, loginFormData, registerFormData,
  handleLoginInputChange, handleRegisterInputChange,
  handleLogin, handleRegister, loading
}) {
  const [termsAccepted, setTermsAccepted] = useState(false);

  // New flow states
  const [promoImageShown, setPromoImageShown] = useState(false); // ipbi.png shown after click
  ...
  const handleHeroClick = () => {
    // If already shown or content visible, just scroll to auth/register
    if (contentVisible) {
      scrollToAuth('register');
      return;
    }

    // show the promo image, then after 3s show modal
    setPromoImageShown(true);
    // ensure modal appears 3s later
    setTimeout(() => {
      setShowModal(true);
    }, 3000);
  };
  ...
  const handleModalClose = () => {
    setShowModal(false);
    // Reveal content below once modal closed
    setContentVisible(true);
    // scroll to main content so user sees the rest
    const main = document.getElementById('main-content');
    if (main) setTimeout(() => main.scrollIntoView({ behavior: 'smooth' }), 200);
  };

  // register submit with terms check
  ...
  // hide default promo modal; we use click flow instead
  useEffect(() => {
    // If the user reloads and contentVisible is true, ensure promoImageShown is false
    if (contentVisible) setPromoImageShown(false);
  }, [contentVisible]);

  return (
    <div className="landing-page">
      {/* hero: click to start promo-image -> 3s -> show modal -> reveal content */}
      <section
        className={`hero-section ${promoImageShown ? 'hero-active' : ''}`}
        onClick={handleHeroClick}
        role="button"
        aria-label="Begin with InvestmentPlus"
      >
        {/* If promo image shown by click, display it full-screen */}
        {promoImageShown ? (
          <img src={promoImage} alt="InvestmentPlus Promotion" className="hero-image promo-image" />
        ) : (
        ...
        <div className="hero-content">
          ...
          <div className="hero-cta-animate">
            <span className="cta-button-text">Tap Anywhere to Begin</span>
          </div>
        </div>
      </section>

      {/* Promo Modal (appears 3s after hero click) */}
      {showModal && <PromoModal onClose={handleModalClose} onRegisterClick={() => { handleModalClose(); scrollToAuth('register'); }} />}

      {/* Only render main page content after promo modal completes (or if user already passed it) */}
      {contentVisible && (
